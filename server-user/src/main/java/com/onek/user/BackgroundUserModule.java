...
//        }
    ...
    private static final String _QUERY_BDUSER_INFO = " select distinct cp.cid companyId, ( SELECT u.uphone FROM tb_system_user u WHERE u.cid = cp.cid ) phone," +
                                                    " ( SELECT u.uid FROM tb_system_user u WHERE u.cid = cp.cid ) uid,cname company,caddrcode addressCode, " +
                                                    " caddr address,cp.auditdate,cp.audittime,cp.cstatus,stu.uid cursorId,stu.urealname cursorName, "+
                                                    " stu.uphone cursorPhone,"+
                                                    " control,cp.storetype storetype  FROM {{?"+DSMConst.TB_COMP+"}} cp, "+
